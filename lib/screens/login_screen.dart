...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _initWebview() async {
    try {
      ...
      await _webviewController.loadUrl(ScheduleLoginFetchService.targetUrl);
      ...
      if (!mounted) return;
      setState(() => _statusText = '浏览器初始化失败: $e');
    }
  }
  ...
  Future<void> _fetchWithSemester(String semester) async {
    ...
    await _loginFetchService.saveSemesterCode(semester);
  ...
  Future<void> _pickTargetSemester() async {
    ...
    if (provider.currentSemesterCode != null && provider.currentSemesterCode!.isNotEmpty) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          if (_isWebviewReady && !_isFetching)
            TextButton.icon(
